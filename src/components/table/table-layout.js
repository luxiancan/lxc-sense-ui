...
class TableLayout {
  ...
  updateScrollY() {
    ...
    if (this.table.$el && bodyWrapper) {
      const body = bodyWrapper.querySelector('.ss-table__body');
  ...
  updateElsHeight() {
    ...
    const headerTrElm = headerWrapper ? headerWrapper.querySelector('.ss-table__header tr') : null;
    const noneHeader = this.headerDisplayNone(headerTrElm);

    // eslint-disable-next-line no-multi-assign
    const headerHeight = this.headerHeight = !this.showHeader ? 0 : headerWrapper.offsetHeight;
    if (this.showHeader && !noneHeader && headerWrapper.offsetWidth > 0 &&
    ...
    this.viewportHeight = this.scrollX ?
      tableHeight - (noData ? 0 : this.gutterWidth) : tableHeight;
