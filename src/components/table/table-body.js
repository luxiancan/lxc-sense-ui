...
import {
  getStyle, hasClass, removeClass, addClass
} from '@/utils/dom';

import { debounce } from 'throttle-debounce';
import SsTooltip from '@/components/tooltip'
...
export default {
        ...
        <tbody>
          ...
          }
          {
            <ss-tooltip
              ...
              placement="top" ref="tooltip" content={ this.tooltipContent }>
            </ss-tooltip>
          }
        </tbody>
      </table>
  ...
  watch: {
    ...
    // update DOM manually.
    // see https://github.com/ElemeFE/element/pull/13954/files#diff-9b450c00d0a9dec0ffad5a3176972e40
    'store.states.hoverRow'(newVal, oldVal) {
      ...
      raf(() => {
        const rows = this.$el.querySelectorAll('.ss-table__row');
  ...
  methods: {
    ...
    getRowClass(row, rowIndex) {
      const classes = ['ss-table__row'];
      if (this.table.highlightCurrentRow && row === this.store.states.currentRow) {
        classes.push('current-row');
      }

      if (this.stripe && rowIndex % 2 === 1) {
        classes.push('ss-table__row--striped');
      }
    ...
    handleCellMouseEnter(event, row) {
      ...
      if (!(hasClass(cellChild, 'ss-tooltip') && cellChild.childNodes.length)) {
        return;
      }
      ...
      if ((rangeWidth + padding > cellChild.offsetWidth ||
        cellChild.scrollWidth > cellChild.offsetWidth) && this.$refs.tooltip) {
    ...
    rowRender(row, $index, treeRowData) {
      ...
      let display = true;
      if (treeRowData) {
        rowClasses.push(`ss-table__row--level-${treeRowData.level}`);
    ...
    wrappedRowRender(row, $index) {
      ...
      const {
        treeData, lazyTreeNodeMap, childrenColumnName, rowKey
      } = store.states;
      if (this.hasExpandColumn && isRowExpanded(row)) {
        ...
        return [[
          ...
          <tr key={`expanded-row__${tr.key}`}>
            <td colspan={ this.columnsCount } class="ss-table__expanded-cell">
