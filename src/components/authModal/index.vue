<template>
    <transition name="dialog">
        <section class="ss-auth-bg ss-auth-dialog" v-show="isShowDialog">
            <div class="ss-select-popup">
                    ...
                    <div class="ss-mian-content">
                        <div class="ss-assign-menber-box">
                            <div class="ss-assign-menber-tree-wrapper">
                                ...
                                <div class="ss-assign-menber-tree">
                                    ...
                                    <div v-if='!assignSearchContent' ref='treeBox'>
                                        <div class="ss-bread-nav" v-if="navList.length&&start==0" ref='breadNav'>
                                            <span v-for='(navItem,navIndex) in navList' :key='navIndex'>
                                                ...
                                                <span class="ss-bread-separator" v-show='navIndex!=navList.length-1'>/</span>
                                            </span>
                                        </div>
                                        <div class="ss-org-tree">
                                            <div class="ss-org-tree-item ss-allchecked" v-if='start==0'>
                                                <input 
                                                    class="ss-filled-in"  type="checkbox" 
                                                    :id="`checkbox`" 
                                                    ref="treeinput" v-model='allChecked' 
                                                    @change="changeAllChecked(allChecked)"
                                            ...
                                            <div class="ss-org-tree-item" v-for="(item,index) in visiableTree" :key='index' ref='treeItem'>
                                                <div class="ss-item-box">
                                                    <div class="ss-org-tree-item-name">
                                                        <input class="ss-filled-in"  
                                                            type="checkbox" 
                                                            :disabled="disabledCheckedFunc(item)" 
                                                            :id="`checkbox${item.uid}`" 
                                                            ref="treeinput" 
                                                            :value='item.cid' 
                                                            v-model="checkedMap[item.cid]" 
                                                            @change="changeChecked(item,false)"/>
                                                        <label :for="`checkbox${item.uid}`" class="ss-firstIndexLabel">
                                                            ...
                                                            <span v-if="item.type=='department'&&isShowDeptNums">({{item.user_num}})</span>
                                                        </label>
                                                    </div>
                                            ...
                                            <div class="ss-loading-msg" v-if="isScrollLoading">加载中</div>
                                        </div>
                                    </div>
                                    <div class="ss-org-tree search-tree" v-if='assignSearchContent' ref='searchBox'>
                                            ...
                                            <div class="ss-item-box">
                                                <div class="ss-org-tree-item-name">
                                                    <input class="ss-filled-in"  
                                                        type="checkbox"
                                                        :disabled="disabledCheckedFunc(item)"
                                                        :id="`checkbox${item.cid}`" ref="treeinput" 
                                                        :value='item.cid' 
                                                        v-model="checkedMap[item.cid]"
                                                        @change="changeChecked(item,false,true)"/>
                            ...
                            <div class="ss-assign-menber-list-wrapper">
                                <div class="ss-assign-menber-list">
                                    <div class="ss-selected-msg-empty" v-if="showAuthEmptyMsg">已选择0人</div>
                                    <div class="ss-assign-choice-wrapper" v-for="(item,index) in choseList" :key="index" :class="{'ss-hideClass':(!isShowLastSelected&&item.isLastSelected)}">
                                         <span v-if="item.type=='user'">
                                            <name-replacer :id="item.employee_id" type='0' :name="item.name"></name-replacer>
                                         </span>
                                         <span v-else>
                ...
                <div class="ss-dialog-footer">
                    ...
                    <button class="ss-confirm-btn" @click="confirmFunc" :class="{'ss-is-disabled':isDisabledDialog||hasClicked}">{{rightButtonText}}</button>
                </div>
            </div>
        </section>
    </transition>
    ...
    export default {
        ...
        data() {
            return {
                ...
                //滚动加载新加
                pageIndex:1,//每次加载的页码数
                isScrollLoading:false,//是否正在滚动加载数据
                pageSize:20,//滚动加载每次请求的数量
                total:0,//memberTree部门加人员的zongshu
                lastUserId:"",//分页的lastId
                deptFlagMap:{},//用于存储每个部门下面需要记住的变量
                userTotal:0,//当前部门下用户的总数
                disabledMap:{},//上一次勾选的禁用map
                cunrrentDepartArr:[],//当前部门下的子部门组合
        ...
        methods: {
            ...
            scrollEvent(){
                ...
                let minBottomHeight=30;//距离底部多少像素开始加载
                let scrollBoxHeight=$(".ss-assign-menber-tree").height();//滚动显示区的高度
                let scrollFunc=that.debounce(function(){
                    let that=this;
                    let scrollTop=scrollItem.scrollTop;
                    let scrollContainerHeight=$(".ss-treeTop").height();
                    let totalHeight=scrollBoxHeight+scrollTop+minBottomHeight;//滚动区+scrollTop
                    if(!that.assignSearchContent){
                        ...
                        if(scrollContainerHeight<=totalHeight){
                            if(!that.isScrollLoading&&(that.memberTree.length-that.cunrrentDepartArr.length)<that.userTotal&&that.lastUserId){
                                ...
                                //模拟分页api
                                get('/training_api/address_book/get_list', params).then(res => {
                                    if(res.code==0){
                                        let data=res.data;
            ...
            clickToNextLevel(item,isNavClick){
                ...
                that.isLoading = true;
                get('/training_api/address_book/get_list', departParams).then(function(treeResponse){
                    if(treeResponse.code === 0){
                        //组装数据
                        let data=treeResponse.data;
                        that.start = 0;
                        that.lastUserId=data.ext.last_id;
                        that.cunrrentDepartArr=data.dept;
                        that.total=data.dept.length+data.ext.total;
            ...
            //用于手动保存上一次选中的人员列表
            manualGetSelected(params){
                let that=this;
                //放入已经指派的所有人员的cid，同步左边组织结构的勾选状态
                let assignedIds=params.ids;
                //同步左边勾选状态
                assignedIds.forEach(function(item){
                    that.checkedMap[item]=true;
                })
                that.assignedUserList=params.list;
                //对应部门下selectedCount++
                that.assignedUserList.forEach(function(assignItem){
                    //用于标识是否是上次勾选的
                    assignItem.isLastSelected=true;
            ...
            circleGetAllTreeByLastId(){
                ...
                if(that.memberTree.length<that.total&&that.lastUserId){
                    ...
                    if(that.isDisabledLastSelected){
                        //上一次选中的不拼接到右边
                        let arr=that.memberTree.filter(function(item){
                    ...
                    that.deptFlagMap[that.topNodeUid].checkedCount=that.total;

                    //circleGetAllTreeByLastId循环调用api完毕关闭loading
                    that.isLoading = false;
                }
            },
            //改变选择状态  isOuterChecked表示是否为外部的删除事件 isSearchChecked 表示是否为搜索的checked事件
            changeChecked(item,isOuterChecked,isSearchChecked){
                let that=this;
                let deleteItem=[];
                //外部的删除事件，that.checkedMap需要手动更新
                if(isOuterChecked){
...
<style lang="scss">
    .ss-auth-bg{
        &.ss-auth-dialog{
            ...
            // .ss-bread-nav > span:last-child .ss-bread-separator {
            //     display: none;
            // }
            .ss-bread-nav{
                margin-right: 18px;
