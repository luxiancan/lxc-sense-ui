...
export default {
  ...
  methods: {
    ...
    changeSize(e) {
      ...
      e.preventDefault();
      let cropperWidth = getComputedStyle(this.$refs.cropper).getPropertyValue("width")
      let scale = this.scale;
      // 放大为 100 缩小为-100
      var change = e.deltaY || e.wheelDelta;
      // 根据图片本身大小 决定每次改变大小的系数, 图片越大系数越小
      var isFirefox = navigator.userAgent.indexOf("Firefox"); // 对火狐的兼容
      change = isFirefox > 0 ? change * 30 : change;
      // 修复ie的滚动缩放
      if (this.isIE) {
        change = -change;
      }
      // 缩小不可以低于cropper组件大小的0.1
      if(change > 0 && (this.trueWidth*this.scale) < (parseInt(cropperWidth) / 10)) return
      // 1px - 0.2
      var coe = this.coe;
      coe =
        coe / this.trueWidth > coe / this.trueHeight
    ...
    changeCropSize(e, w, h, typeW, typeH) {
      e.preventDefault();
      window.addEventListener("mousemove", this.changeCropNow); // 指针在元素内移动时持续触发。
      window.addEventListener("mouseup", this.changeCropEnd); // 释放的时候触发
      window.addEventListener("touchmove", this.changeCropNow);
      window.addEventListener("touchend", this.changeCropEnd);
      this.canChangeX = w;
      this.canChangeY = h;
      this.changeCropTypeX = typeW; // 改变的基准点
      this.changeCropTypeY = typeH; // 改变的基准点
      // 兼容移动端
      this.cropX = "clientX" in e ? e.clientX : e.touches[0].clientX;
      this.cropY = "clientY" in e ? e.clientY : e.touches[0].clientY;
